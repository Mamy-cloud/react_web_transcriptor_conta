  ...
  const handleSTT = async () => {
    ...
    try {
      const res  = await apiFetch(API.STT_SEGMENTS, {
        method: 'POST',
        body:   JSON.stringify({ url_audio: fichier.url_audio }),
      })
      ...
      if (data.success) {
        const segments: Segment[] = data.segments.map((s: any, i: number) => ({
          ...
          debut:          s.debut,
          fin:            s.fin,
          statut:         'en-attente' as SegStatut,
      ...
      <div className="trans-page">
        <div className="trans-inner">
          ...
          <div className="trans-file-bar">
            ...
            >
              <option value="" disabled>Sélectionner un fichier…</option>
              {fichiers.map(f => (
                <option key={f.id} value={f.id}>
                  {f.nom} — {f.temoin} ({f.duree})
                </option>
              ))}
            </select>
          </div>
            ...
            <>
              ...
              <div className="trans-player">
                <div className="trans-player-info">
                  ...
                  <div className="trans-player-meta">{fichier.temoin} · {fichier.duree}</div>
                </div>
                <div className="trans-player-controls">
                  ...
                  <div className="trans-timeline">
                    ...
                    <span className="trans-timeline-time">00:00 / {fichier.duree}</span>
                  </div>
                </div>
